...
def add_rotation_error(se3_matrix, rotation_error):
    ...
    rotation_error_matrix = rotation_error_matrix.view(
        -1, 3, 1
    )  # Reshape for batch processing
    rotation_error_matrix = (
        torch.eye(3).to(rotation_error_matrix.device)
        + torch.sin(rotation_error_matrix)
        + (1 - torch.cos(rotation_error_matrix))
    )
...
def get_pointcloud(
    ...
    def remove_far_pixels(depth, threshold_factor=2.0):
...
def initialize_first_timestep(
    ...
    mask_tmp = (depth > 0) & (depth < 2.0)  # Mask out invalid depth values
...
def get_loss(
    ...
    if VIS_LOSS_IMAGE and (iter_time_idx + 1) % 50 == 0:
        # if False:
        ...
        diff_candy = (
            torch.abs(weighted_render_candy - weighted_candy).mean(dim=0).detach().cpu()
        )
        diff_sil = (
        ...
        if tracking_iteration:
            suptitle = (
                f"frame_id: {curr_data['id']}, Tracking Iteration: {tracking_iteration}"
            )
        else:
